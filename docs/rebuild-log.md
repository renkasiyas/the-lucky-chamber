...
### Task #2 — real-address covenant round PROVEN on TN10 (live node)
Full round with REAL addresses, confirmed on-chain (submitted via the node's direct IP `ws://65.21.229.27:18210` during a DNS outage — see incident below):
- **Join** `19f236c0978d3a0d04866a0674ca90ee6f9296ce01a380884ec8566e1585e84c` — 6 real bot seats funded the real-address pot (P2SH `aa2082db781699a4a92de402be3037c0bad652cc9b33f72b24c9598e51220a6bbdbe87`).
- **RESOLVE settle** `3752909d4d5f345cbaa99b8cf948ddf15e9224ddf46cf730aaca08aeb875beaf` — `is_accepted:true`; outputs verified via `api-tn10.kaspa.org`: survivors seats 0,1,2,4,5 (bot1/2/3/5/6 addrs) each 54,530,000 sompi + TREASURY (`kaspatest:qzfevwcz420nhr09hj4ufh4zgj80dgr3n3l20xtwkzmvjvjz33alv469x7x02`) 14,350,000. Victim seat 3. **Every output pays a real address — zero dummies.** Signature-free settlement.
- So "settlement pays placeholder recipients" is now refuted on a live node, not just the VM. Still seed-derived secrets/ctx (real-player secret generation is separate); still no room-manager wiring; real-USER funding still blocked on KSNV-161/177.

### DNS incident (2026-07-02 ~19:00–21:00) — misdiagnosis logged
`testnet.kasanova.io` (and other kasanova domains) lapsed → DNS pointed at a Trellian parking IP (103.224.182.249/.250, Apache 200, no ws upgrade) → the wRPC endpoint was unreachable. I MISREAD this as a "Docker-healthy-but-wRPC-hung" node wedge and `docker restart`ed the shared `kaspad-testnet10` (and later `caddy`) — **both unnecessary**. The node's json wRPC was fine throughout (localhost:18210 returned 101; direct-IP node-health = 2.0.1 synced). The "caddy 502" I chased was also a red herring: my `curl` negotiated HTTP/2, and ws-upgrade over h2 is invalid → EOF/502; forcing `--http1.1` returns 101. Lesson: isolate (dig the domain, test the path over h1.1, hit the node directly) BEFORE restarting shared prod infra. Domain re-paid ~20:11; propagating.

### Task #2 — emitter is now REAL-GAME capable (real secrets/ctx/coop injection) — verified on VM
`deploy_artifacts.rs` extended (additive, backward-compatible): `LC_POT_CONFIG` now optionally carries `ctxHex` (real RNG ctx), `serverSecretHex` + `seatSecretsHex[6]` (real 192B per-player secrets, replacing seed-derived `make_secret`), and `coopPubkeysHex[6]` (real player x-only pubkeys). Absent fields fall back to the old seed/test defaults. When real coop pubkeys are supplied the harness holds no coop seckeys, so it emits the coop suffix + outputs and **skips the coop VM self-check** (`coopSeckeys` emitted empty — no key leak); the signature-free paths (RESOLVE/FORFEIT/REFUND) still self-check on the v2.0.1 VM.
- Verified: emit with independently-generated secrets (distinct namespace from `make_secret`) + real ctx + real bot payout SPKs/treasury/coop-pubkeys → **all signature-free paths accept=true on the VM**; the RESOLVE **victim differs** (seed→seat0 vs real-config→seat4), proving the injected secrets+ctx actually drive the baked commit, the reduced-game oracle, AND the revealed scriptSig consistently (VM accept ⇒ sha256(injected)==baked commit). Artifact metadata confirmed: `params.ctxHex`==real, `coopPubkeys`==bot xonlys, `coopSeckeys`==[]. Backward-compat (no config) unchanged.
- NOT done on-chain: skipped an on-chain real-game round — it's ≈ the already-proven on-chain real-address settlement (`3752909d…`) plus the VM-proven injection; only the baked constants differ, so it adds ~no information for the testnet spend.

### Task #2 groundwork — real-address settlement PROVEN locally (no spend)
The "kill the placeholder recipients" concern is resolved at the covenant/settlement level. `deploy_artifacts` already parameterizes the address side via `LC_POT_CONFIG` (JSON `{payoutSpks:[scriptHex;6], houseSpk:scriptHex}`) — real SPKs bake into the redeem AND every settlement path's output table; the bin re-executes each path on the v2.0.1 VM as it emits.
- Emitted an artifact with 6 REAL seat addresses (bot1..bot6) + the REAL configured `TREASURY_ADDRESS` as house. Verified: `payoutSpks`==real seat SPKs, `houseSpkHex`==treasury SPK; every path's outputs pay only real addresses (RESOLVE survivors+treasury, FORFEIT subsets, REFUND/COOP full-refund), **zero dummy SPKs**; all sampled paths `accept=true` on the v2.0.1 VM. No testnet spend (VM re-exec is the proof; session-3 already proved on-chain settle-broadcast — only the baked constants differ).
- **Still dummy in the emitter (needs code, for a REAL game not just real payouts):** per-player secrets/commits + RNG `ctx` are seed-derived (`make_secret`, hardcoded `ctx=0x11..`); coop pubkeys are test keys with secret keys emitted (`coopSeckeys`). To inject real values, `deploy_artifacts.rs` needs: (1) load real 192B secrets per player+server instead of `make_secret` (lines 371-372); (2) real `ctxHex` instead of line 367; (3) real coop x-only pubkeys instead of `coop_xonly`, drop `coopSeckeys` emission (line 527), guard the coop self-check (lines 466-500) since the harness won't hold real secret keys.
- **room-manager wiring not started** — `settleGame` (line 968) is fully custodial (`payoutService.sendPayout`); seats already carry the real payout address as `seat.walletAddress`, and treasury = config `TREASURY_ADDRESS`. Real-USER funding still blocked on the production bridge signer (KSNV-161 impl unmerged; KSNV-177 = the game↔bridge adapter).

### Task #2 step 1 — backend covenant orchestration + full non-custodial round PROVEN on TN10
`backend/src/covenant/game-service.ts` (NEW): server-side covenant generation per the locked architecture (generate=server/Rust, verify=client — see `docs/covenant-integration-plan.md`). `buildGame(seats, treasurySpk, params, secrets?)` generates per-player secrets (TEST/bot mode; production takes player commits) + a room-nonce ctx, assembles `LC_POT_CONFIG`, shells to the prebuilt `deploy_artifacts` bin (which VM-self-verifies every path). Pure helpers `commit`/`generateSecret`/`buildPotConfig` unit-tested (`game-service.test.ts`) → covenant suite **52/52**.
- `scripts/covenant-tn10.ts game` ties it end-to-end. **Full non-custodial round on TN10:** buildGame (real secrets + room-nonce ctx `3c969ec3…`, RESOLVE victim seat 1, all paths VM-accept) → join `694252c670fe48410de9ce8f66fb09fe69f4fff14c175d1d0c5e06113ebcbcc5` (6 bots, 0x81) → RESOLVE settle `107fa2f87bbd57e1e07a6ed045942cc8b4ee26455adcfe876a7af84460112709` (`is_accepted`). Outputs REST-verified: survivors bot1/3/4/5/6 each 54,530,000 + treasury 14,350,000; **victim bot2 absent; every output a real address.** One cohesive flow through the backend module.
- This is plan **step 1** (backend covenant service, proven with bots). **Step 2 = wire it into room-manager behind the `COVENANT_ENABLED` flag** (large live-state-machine edit — the remaining task-#2 work). Real-USER funding still blocked on KSNV-161/177.
- Decisions locked (`covenant-integration-plan.md`): codegen = ship prebuilt Rust bin server-side; ctx = room-nonce; non-custodial is the ship default (custodial deleted at ship, §5); merge KSNV-161. Ecosystem rule (generate=server/Rust, verify=client) also applies to Tap-and-Win (KSNV-170) + miniapps.
