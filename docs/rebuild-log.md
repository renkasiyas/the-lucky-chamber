...
- **PROVEN (broadcast + confirmed, txid):** node health; funds; **RESOLVE** end-to-end (`3a32d35d…`→`4d734453…`, outputs==oracle); **S3 D1-boundary** (FORFEIT `744781473e…` rejected pre-D1 / accepted at D1, window ~108s) which also settles a **FORFEIT subset**; **S5 malleation** (inflated `sigops=255` self-rejects, honest `sigops=2` confirms `43ed457c…`, same txid).
- **IN PROGRESS:** S6 KIP-20 indexing, COOP-ABORT (on-chain re-sign via sig-splice), D2 REFUND, spec-§2 ANYONECANPAY fund-join broadcast.
